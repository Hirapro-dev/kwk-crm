/**
 * 出金管理-親 一覧画面 (CLAUDE.md §5.13 / §8.1)
 *
 * 償還枠(親)を一覧表示する。閲覧は admin/manager/support のみ(RLSでも制限)。
 * 一覧カラムはオブジェクト管理(/settings/objects/withdrawal_parents)の設定に従う。
...
import { listWithdrawalParents } from '@/lib/domain/withdrawals';
...
export default async function WithdrawalParentsPage({ searchParams }: PageProps) {
  ...
  const [result, listFields] = await Promise.all([
    listWithdrawalParents({ q: sp.q, sort: sp.sort, dir, page: 1, pageSize: LIST_PAGE_SIZE }),
    getVisibleFields('withdrawal_parents', 'list'),
  ]);
  ...
  return (
    <div className="space-y-3">
      <Card className="overflow-hidden p-0 shadow-sm">
        ...
        <div className="flex items-start justify-between border-b px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <h1 className="text-base font-bold text-foreground">出金管理-親</h1>
        ...
        {/* 検索帯(償還-親No / 会員ID / 会員氏名 / 投資案件 を部分一致) */}
        <div className="border-b px-4 py-2" style={{ backgroundColor: '#f9f9f9' }}>
          <form method="get" className="flex items-center gap-2">
            <input
              ...
              name="q"
              defaultValue={sp.q ?? ''}
              placeholder="償還-親No・会員ID・会員氏名・投資案件で検索"
            ...
            {sp.q ? (
              <Link href="/withdrawal-parents" className="sf-link text-sm">
                クリア
              </Link>
            ) : null}
          </form>
