 ...
 * - スマホ(md 未満): 左メニューは普段は出さず、メニューボタンで本文の上に重ねて開く(背景タップか画面遷移で閉じる)
 */
...
export function TaskShell({
  ...
  return (
    <div className="flex h-dvh flex-col bg-background">
      <TaskTopbar me={me} onMenuClick={toggle} menuOpen={mobileOpen} />
      <div className="relative flex min-h-0 flex-1">
        ...
        <div
          className={cn(
            ...
            mobileOpen ? 'fixed bottom-0 left-0 top-12 flex' : 'hidden',
            collapsed ? 'md:hidden' : 'md:flex',
          )}
        >
          <TaskSidebar projects={projects} folders={folders} canCreate={canCreate} />
        </div>
        <main className="min-w-0 flex-1 overflow-y-auto bg-[#f7f8fa] p-3 md:p-4">{children}</main>
      </div>
    </div>
  );
}
