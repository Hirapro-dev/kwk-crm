...
| URL | 画面 | 内容 |
|---|---|---|
| `/mail` | メーラー(一覧) | 2026-09-09 変更: CRM 本体とは別画面(`app/(mailer)`)。アプリランチャー/歯車メニューの「メーラー」から別タブで開く。メールディーラー風の左: 受信箱フォルダ(ドメイン > アドレス、未対応件数) / 右: 一覧(`InfiniteTable` 再利用。列: 状態 / 件名 / From / 受信箱 / 日付 / 担当)。上部に状態タブ(新着 / 対応中 / 対応完了 / すべて / メルマガ / 自動応答 / 迷惑メール)。分割ビューは廃止 |
...
| `/mail/settings` | 設定 | メーラーのヘッダーの歯車メニューから開く。受信箱(mail_boxes)の追加・編集、送信ドメインの SES 登録と DKIM CNAME の表示。admin のみ(M2) |
...
| **M2 送信** | 返信・新規送信、配信状態反映、署名、`/mail/settings` | 2日 |
