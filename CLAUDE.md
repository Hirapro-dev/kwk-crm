...
- `gender` text — 性別。実データは「男 / 女 / 法人」(CSV 取込由来)。画面(会員編集ダイアログ・会員一覧の新規登録・問合せからの新規会員登録)では
  **選択式**(表示は 男性 / 女性 / 法人 / その他、保存値は 男 / 女 / 法人 / その他。CSV の定期取込で入る「男 / 女」と混在させないため保存値は変えない。
  `lib/domain/member_gender.ts` の `GENDER_OPTIONS`。現在の値が選択肢に無ければその値を選択肢に足して失わない。2026-09-17)
- `birthdate` date
...
- **取込は 2 方式**(設定 → データ取込のオブジェクト選択):
  ...
  2. **クリック履歴 CSV**(2026-09-23。`import_article_reaction_clicks.ts`。配信ツールの書き出し「クリック日時 / リンクNo / 読者No /
     読者メールアドレス / 読者名前」。Shift_JIS は取込画面で自動判定)。取込画面で **備考(記事名。必須)** と **配信媒体**(既定の 3 値 KAWARA版一般会員 / KAWARA版正会員 / 仮想通貨長者 + DB にある値。`ARTICLE_REACTION_MEDIA_DEFAULTS`。or 直接入力)、
     **日付**(任意。2026-09-23 追加。**既定は日本時間の昨日** `jstYesterday`。2026-09-25)を指定してから取り込む。**同じメールアドレスは 1 件にまとめる**(登録日時 = いちばん早いクリック、氏名 = 空でない最初の値。純粋関数 `dedupeClickRows`)。
     **同じメール + 同じ備考が既にあれば作らない**(プレビューで「スキップ」。DB 側も部分ユニーク `uq_artreact_email_remarks`)。
     **Salesforce 形式の既存行との突合**(2026-09-23): 同じ記事名(詳細)・同じ日付の既存行(メール列なし)で、その会員のメール(email1〜3)がクリック行の
     メールと一致する、または会員氏名が読者名前と一致する(空白を除いて比較。メール一致を優先)ものは「同じ反応」として作らず、既存行に
     `email` と `remarks`(記事名)を書き込む(空のときだけ)。プレビューは「既存(SF取込分)」の件数とサンプルに一致した既存 ID を出す(`legacyMatchedCount`。
     純粋関数 `matchLegacyReactions`)。
...
- **一括の会員検索**(2026-09-23): 一覧の左端チェックで選び、選択中バーの「会員を検索(メール一致)」で、メールアドレスが会員の email1〜3 のどれかと
  **完全一致**(小文字化)した行に会員ID・会員氏名を入れる。メールで当たらない行は、読者名前(`member_name`)が会員氏名と一致(空白を除いて比較)する
  会員が **1 人だけ**なら紐付ける(同日追加。配信ツールに登録したメールが CRM の会員と違う人のため。候補は `loadMemberCandidatesByNames`、判定は同じ純粋関数。
  取込時の照合も同じ)。同名が複数なら複数候補で止める。同じメールの会員が複数いる行(複数候補)・該当なし・メールなし(Salesforce 形式の行)は変えず、
...
部分一致。2026-09-16)。全体検索の各行には右端に登録日時(会員・問合せ・LP)/ 申込日(申込)を出す(2026-09-23)。メニューは「LP」(問合せの次、全ロール)。`object_definitions` に `lp_entries`(sort 92)。
- `source_mail_message_id` text unique nullable — メール取込(§5.16。取込先が LP のルール)で作った LP の元メール(migration 99)。
  ...
  初期ルール 2 本(フォーム名に「LP」/「メールマガジン」)を作成し、旧ルール 11(コーポレートサイト経由のメルマガ → 問合せ)は無効化。
  ルール 11 で問合せとして作られていた 6 件は同じ TA- ID のまま LP へ移し、問合せ側は論理削除した(migration 100。本番適用済み)。
  同日、過去メール(2026-01〜09)の未処理分を一括処理して LP 2,002 件を作ったが、CSV 取込分(9/9 まで)と同じ
  メール・フォーム名・登録日の 1,176 件は重複のため論理削除し、メール側の参照は CSV 側の LP に付け替えた(migration 101。本番適用済み。
  ユーザー決定: CSV 側 = Salesforce の元 ID を残す)。「資産分析AI「極」」系のルール(id 15。フォーム名に「資産分析AI」を含む)も追加。
...
- `task_mentions` — **メンションと受信トレイ**(2026-09-22 追加, migration 115)。コメントで「@氏名」と書くと、その人宛に 1 行できる
  (`comment_id` FK → task_comments / `task_id` / `user_id`(呼ばれた人)/ `created_by`(書いた人)/ `read_at` / `created_at`。UNIQUE (comment_id, user_id))。
  呼ばれた人の判定は純粋関数 `extractMentionUserIds`(本文の「@氏名」を CRM ユーザーの氏名と空白を無視して突き合わせ、長い氏名を優先、本人は除く)で、
  `addTaskComment` がサービスロールで行を作る。RLS は自分宛の行だけ読める・既読にできる。コメント欄は「@」で候補を出す `MentionTextarea`
...
- 書込: viewer 不可。閲覧できるプロジェクトなら INSERT / UPDATE 可(担当者以外も編集できる。Asana と同じ)。
  ...
  プロジェクトの削除は論理削除(`deleted_at`)で、中のタスクもまとめて論理削除する(Server Action `deleteTaskProject`。書込みはサービスロール。
  設定ダイアログの「プロジェクトを削除」。2026-09-18)。
...
**画面**(`/task` 配下。メーラーと同じ独立画面。CRM のメニューバーには出さない(2026-09-18 変更)。§8.1):
- `/task` — **ホーム**、`/task/my` — **マイタスク**: 自分が担当の未完了タスクを 期限切れ / 今日 / 今後 7 日 / それ以降 / 期日なし に分けて一覧。行内で完了チェック・期日変更。完了済みは切替で表示。
...
- `mail_boxes` — 共有アドレス (1行 = 会社側の公開アドレス1つ。数百件を想定)。`id` serial PK /
  ...
  `default_signature_id` int FK → mail_signatures nullable(**既定の署名**。migration 105)/ `is_active` boolean。まず1行 (`ad@kawaraban.co.jp`)。
  ※ 旧列 `signature` text は migration 105 で署名マスタへ写した後は未使用(デプロイ順の都合で列は残す)。
- `mail_signatures` — **署名マスタ**(2026-09-18 追加, migration 105)。署名を受信箱ごとの 1 列ではなく、名前を付けて別に管理し、
  複数の受信箱で使い回す。`id` serial PK / `name` text(署名名。選択肢の表示名)/ `body` text(本文、複数行。上限 2,000 字)/
  `is_active` boolean / `created_at` / `updated_at`。物理削除はしない(受信箱の既定署名が参照するため無効化で代替。マスタ管理 §5.18 と同方針)。
  RLS: SELECT 全ロール(返信フォームで使う)/ 書込 admin。純粋関数は `lib/domain/mail_signatures.ts`(`defaultSignatureValue`:
  受信箱の既定署名 → フォームの初期選択。未設定・存在しない・**無効化済みなら「署名なし」**にして無効な署名を黙って付けない)。
  Server Action は `lib/domain/mail_signature_actions.ts`(追加・編集。admin)。受信箱への紐付けは `updateMailBox` の `defaultSignatureId`。
  既存の受信箱の署名(本番 1 件)は migration 105 で署名マスタに写し(名前は差出人表示名、無ければアドレス)、その受信箱の既定署名にした。
...
※ ヘッダーに Message-ID が無い行(受信メールの一部。全期間で約 1,300 件)は、以前はランダム UUID(`@crm.local`)を ID にしていたため
差分取込のたびに別 ID で再登録され重複していた(2026-09-17 に発覚。再登録分 1,306 スレッドは論理削除で掃除済み)。同日から
メールディーラーの「メールID / 枝番」で決定論的な ID(`<md-<メールID>-<枝番>@maildealer.local>`。純粋関数
`maildealerSyntheticMessageId`)にし、以前のランダム ID で入っている同じ行は内容キー(送受信日時・差出人・方向・件名。
`maildealerContentKey`)で突き合わせて既存扱いにする。
...
- 本文の解析: `text_body`(無ければ `html_body` をテキスト化)を行に分け、「ラベル: 値」(半角/全角コロン)を辞書にする。
  **「ラベル:」だけの行**(値が次の行以降にある複数行の項目。例: 未来予測レポート(リクエスト)の「銘柄詳細:」)は、次の空行までの行を改行で連結して 1 つの値にする
  (先頭の見出し行「▼…」は除く。まとめた行は個別のラベルとしては扱わない。2026-09-23)。
...
- **既存ルールを手で当てはめる**(2026-09-17): どのルールにも一致しない(未設定)メールに、admin が既存ルールを選んで適用できる。
  一覧の「取込ルール」列(`ApplyImportRuleCell`)、チェックした複数行への一括操作(`MailBulkActions`「取込ルールを当てはめる」、1 回 500 件)、
  メール詳細の「取込ルール」パネル(`allRules`)の 3 か所。選んだ時点で**そのルールに一致しなかった理由**(純粋関数 `ruleMismatchReasons`:
  受信箱が違う / 差出人が違う / 件名・本文・フォーム名にキーワード「…」を含まない / フォーム名を取得できない / ルールが無効)を注釈で出し、
  適用後は処理結果にも「手動適用「ルール名」(条件不一致: …)」として残す(`importMailMessage` の `forceRule`。
  Server Action `applyImportRuleToThreads` / `previewImportRuleMismatch`)。当てはめは 1 通ずつの救済で、同じ型を今後も自動で取り込むには
  ルールの条件を直す(注釈がその手がかり)。
...
Supabase RLSで以下を実装:
- `members`: SELECT は全ロール全件(migration 33)。UPDATE は **viewer 以外の全ロールが全会員**(2026-09-18, migration 104。それまでは admin か自分が永久担当の会員のみで、永久担当の約 95% が未設定のため実質 admin 専用だった)。プロテクト者・日程は Server Action で admin のみ、削除は admin のみ
...
| `/inquiries` | 問合せ一覧 | フィルタ(フォーム種別/期間/未対応)、検索、会員化ボタン。**メール取込分のみ**(`?mail=1`。§5.16 のリード一覧)では行の先頭に「会員照合 / 操作」列: 照合状態(会員化済 / 候補あり / 該当なし / 確認済み / 未照合)と、①会員検索(自動照合の候補を一致項目つきで表示 + 手動検索 → 紐付け。候補・検索結果の氏名/会員IDは会員詳細へのリンクで別タブで確認できる。2026-09-16。**「この会員に紐付け」を押すと、紐付ける前にその会員の編集フォーム(会員詳細の編集ダイアログと同じ部品 `MemberEditDialog` の制御モード)を開き、会員側が空だった項目に問合せの値(カナ・メール・電話・郵便番号・住所・広告ID)を初期値として差し込む。メールは email1〜3、電話は phone1・電話番号2・3 の空き枠に入れ、既にある値は上書きしない(純粋関数 `inquiryOverridesForMember`)。利用者は会員編集と同じ項目を直せ、「保存して紐付け」で会員を更新してから紐付ける(`loadMemberForLink` / `linkInquiryToMember(…, memberPatch)`)。2026-09-17**)②新規会員登録(氏名・メール・電話を引き継ぎ、K- 採番。2026-09-16 から 広告ID / 広告媒体名(【取得】で広告マスタから)/ 個人情報取得ポイント(マスタから選択)/ 顧客情報取得日(既定は今日)/ メルマガ登録日時 も指定できる。2026-09-17 から 性別(選択式)/ 郵便番号 / 住所(問合せの値を初期値に編集可)も。`convertInquiryToMember` の `member_fields`)③確認済み(会員を作らずに確認済みにする)。`app/(app)/inquiries/LeadActions.tsx` / `lib/domain/inquiry_lead_actions.ts` / 純粋関数 `lib/domain/inquiry_lead.ts` |
| `/lp` `/lp/[id]` | LP 一覧 / 詳細 | LP・メルマガ登録系フォームの問合せ(§5.17。問合せとは別オブジェクト、取込専用)。検索・フォーム名の絞り込み・無限スクロール。会員が紐付く行は会員詳細へリンク |
| `/inquiries/[id]` | 問合せ詳細 | フォーム固有情報表示、メモ。**「編集」**(viewer 以外。2026-09-18): 項目管理の「詳細」表示ONの項目をセクション順に並べた編集ダイアログ(`InquiryEditDialog`。会員編集と同方式)。DB 列はホワイトリスト(`EDITABLE_INQUIRY_COLUMNS`: フォーム(選択)・氏名・カナ・メール・電話・郵便番号・住所・広告ID(【取得】)・登録日時・**会員ID**(会員検索から選択 or 直接入力、解除可。保存時に実在確認。2026-09-18))、可変項目は定義済みキーだけ差し替え(純粋関数 `mergeInquiryExtra`。備考は既存のインライン編集)(Server Action `updateInquiry`)。会員化の操作は一覧(メール取込分)と同じ `LeadActions`(2026-09-16 に統一): **会員検索**(自動照合の候補を一致項目つきで表示 + 氏名/カナ/メール/電話/会員IDの手動検索。スクロールして既存会員かどうかを確認し、その会員に紐付け)/ **新規会員登録**(氏名・メール・電話・住所を引き継ぎ、K- 採番)/ **確認済み**。旧 `ConvertButton`(会員IDの直接入力)は廃止 |
| `/members` | 会員一覧 | フィルタ(担当/種別/期間)、CSV出力。ヘッダー右の**「新規登録」**(viewer 以外。2026-09-16)で問合せを経由せずに会員を作成: 氏名(必須)・カナ・性別(選択式。2026-09-17)・メール・電話・郵便番号・住所・広告ID/広告媒体名(【取得】で広告マスタから)・個人情報取得ポイント(マスタから選択)・顧客情報取得日(既定は今日)・メルマガ登録日時・担当(既定は自分)。同じメール/電話の会員があれば一度止めて候補(会員詳細へのリンク付き)を出し、「別人」を確認したときだけ登録できる。ID は `gen_member_id()`。登録後は会員詳細へ(Server Action `createMember`) |
| `/members/[id]` | 会員詳細 | 基本情報、申込履歴、活動履歴タイムライン、活動追加。**編集ダイアログは viewer 以外の全ロール**(2026-09-18 に admin 限定から開放。RLS も migration 104 で viewer 以外は全会員を更新可に。プロテクト設定はダイアログ内で admin のみ、削除は admin のみ)。対応歴は**接触種別(チェックボックスで複数選択 = いずれかに一致。例: アウトとインだけ)・状態(通電/不在/接触対応/申込獲得/受信/送信)・期間**で絞り込める(サーバー側で絞り、先頭ページを読み直す。`MemberActivityTimeline`。メール由来の対応歴(§5.7)が増えたため。2026-09-16) |
| `/applications` | 申込一覧 | フィルタ(案件/ステータス/担当)。ヘッダー右の**「新規登録」**(viewer 以外)で申込を作成: 会員を検索して選択(必須)・案件(必須)・申込日(既定は今日)・ステータス(既定 対応中)・区分・申込獲得者・**契約書送付日・利息(`interest`。円金利 `yen_interest` は出さない)・起算日時(`start_datetime`。日本時間)・契約期日(`contract_end_date`)**・契約期間(●ヶ月)・入金日/入金額。担当はフォームに出さず登録者を入れる(詳細で変更可)。入金予定日/予定額はフォームから外した(2026-09-18 変更。それまでは 担当(既定 自分)・入金予定日/予定額があり、契約書送付日・利息・起算日時・契約期日は無かった)。登録後はその申込の詳細へ移動(Server Action `createApplication`、ID は `gen_application_m_id()`。2026-09-16) |
...
| `/task` | タスク管理(ホーム) | タスク管理(§5.20)。**メーラーと同じく CRM 本体とは別画面**(`app/(tasks)` ルートグループ、独自ヘッダー。メニューバーには出さない: migration 103 で nav_items の `task` を削除)。ヘッダーのタスクアイコン(メールアイコンの右)とアプリランチャーの「タスク」から**別タブ**で開く。Asana 風に**左: メニュー(ホーム / マイタスク / **マイフォルダ**(§5.20 migration 110。プロジェクトをドラッグ&ドロップで整理)/ 参加プロジェクトの一覧、＋で作成)/ 右: 一覧**(`TaskSidebar` / `TaskTopbar`)。**左メニューは開閉できる**(`TaskShell`。ヘッダー左端のメニューボタン。PC は畳む/出すを端末ごとに記憶、スマホ(md 未満)は普段は出さず本文の上に重ねて開き、背景タップか画面遷移で閉じる。一覧の行はスマホではタスク名を 1 行目、担当・期日などを 2 行目に折り返す。2026-09-22)。分割ビューはスマホでは右側ではなく**一覧の代わりに詳細を全幅で出し**、「閉じる」で一覧へ戻る。**スマホは Asana のモバイルアプリ風**(2026-09-22): 画面下に固定タブ(ホーム / マイタスク / 受信トレイ / 検索 / メニュー。`TaskMobileTabBar`。プロジェクト一覧はメニューまたはホームから。「メニュー」で左メニューを重ねて開く)、大きい見出し、一覧の行は 名前 / 2 行目(プロジェクト名 または 担当 · 会員)と期日バッジ(`DueBadge`。編集は詳細で)、マイタスクの期日グループは見出しで開閉。検索ページ `/task/search`(タスク名・プロジェクト名の部分一致。`searchTasks`。PC でも使える)。行内の担当・期日・セクションの入力はスマホでは大きめ(h-9・16px。iOS の自動ズームも防ぐ)、PC では従来どおり小さく(2026-09-22)。ホームは挨拶・未完了/期限切れ/今日の件数・期日が近いマイタスク・参加プロジェクトのカード(2026-09-18) |
...
| `/mail/settings` | メール設定 | admin のみ (M2)。各セクション(受信用アドレス / 受信箱 / **署名** / 送信ドメイン / メール取込ルール)は**アコーディオン**(見出しクリックで開閉。既定は閉じる。開閉状態は端末ごとにブラウザへ記憶。`CollapsibleCard`。2026-09-15)。「その他(未登録アドレス宛)」の説明と「再振り分けを実行」ボタンあり(§5.15)。**メーラーのヘッダーの歯車メニュー**から開く(CRM の /settings 配下ではなくメーラー内。非 admin は /mail へ戻す)。(1) 各サーバーの転送設定に貼る**受信用アドレス**と手順の表示 (2) **受信箱(mail_boxes)の追加・編集**(差出人表示名の既定値 / **既定の署名**(署名マスタから選択)/ 有効・無効。アドレスは受信判定キーのため変更不可、削除もしない)(2') **署名**(署名マスタ `mail_signatures` §5.15 の追加・編集: 署名名 / 本文 / 有効・無効。既定にしている受信箱の数を表示。削除はしない。2026-09-18) (3) **送信ドメインの SES 登録**(`ses:CreateEmailIdentity`、Easy DKIM)と DNS に貼る **DKIM の CNAME 3本**の表示・検証状態・再確認。DNS 追加と各サーバーの転送設定は API が無いため手作業(画面に案内)。差出人表示名の既定値はメーラーの返信・新規作成フォームの初期値になり、送信者が送信時にその場で書き換えることもできる(既定値自体は変わらない) |
