...
| `/inquiries/[id]` | 問合せ詳細 | フォーム固有情報表示、メモ。**「編集」**(viewer 以外。2026-09-18): 項目管理の「詳細」表示ONの項目をセクション順に並べた編集ダイアログ(`InquiryEditDialog`。会員編集と同方式)。DB 列はホワイトリスト(`EDITABLE_INQUIRY_COLUMNS`: フォーム(選択)・氏名・カナ・メール・電話・郵便番号・住所・広告ID(【取得】)・登録日時)、可変項目は定義済みキーだけ差し替え(純粋関数 `mergeInquiryExtra`。備考は既存のインライン編集)。会員の紐付けは変えない(Server Action `updateInquiry`)。会員化の操作は一覧(メール取込分)と同じ `LeadActions`(2026-09-16 に統一): **会員検索**(自動照合の候補を一致項目つきで表示 + 氏名/カナ/メール/電話/会員IDの手動検索。スクロールして既存会員かどうかを確認し、その会員に紐付け)/ **新規会員登録**(氏名・メール・電話・住所を引き継ぎ、K- 採番)/ **確認済み**。旧 `ConvertButton`(会員IDの直接入力)は廃止 |
