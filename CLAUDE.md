...
- **ルールの設定はメールの詳細から行う**(2026-09-15 決定): 取込候補からメールスレッド(`/mail/[id]?folder=candidates`)を開くと
  ...
  (選択肢は問合せオブジェクトの全項目 = DB 列のホワイトリスト + 項目管理で定義済みの可変項目。同名の可変項目が
  あれば初期値でそれに割り当て、無ければ「新しい可変項目として追加」も選べる)を設定し、
